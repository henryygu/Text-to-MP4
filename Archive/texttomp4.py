...
from moviepy.editor import TextClip, ImageClip, ColorClip
import logging
...
for filename in sorted_files:
    ...
    if filename.endswith(".txt"):
        ...
        for i, sentence in enumerate(sentences, 1):
            ...
            if i > highest_i:
                if i == 1:
                    ...
                    centered_image = ImageClip(
                        os.path.join("Images", "hp2.jpg")
                    ).set_position(("center"))
                    result_clip = CompositeVideoClip(
                        [bg_image, centered_image, video_start]
                    )
                ...
                if len(sentence) != 0:
                    ...
                    centered_image = ImageClip(
                        os.path.join("Images", random_file)
                    ).set_position(("center"))
        ...
        final_file_save_loc = os.path.join(
            "Output", f"{os.path.splitext(filename)[0]}.mp4"
        )
